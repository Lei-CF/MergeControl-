...
from envs.common.action import action_factory, Action, DiscreteMetaAction, ActionType
from envs.common.observation import observation_factory, ObservationType
from envs.common.finite_mdp import finite_mdp
from envs.common.graphics import EnvViewer
from highway_env.vehicle.behavior import IDMVehicle, LinearVehicle
from highway_env.vehicle.controller import MDPVehicle
from highway_env.vehicle.kinematics import Vehicle
from envs.common.idm_controller import idm_controller, generate_actions
from envs.common.mdp_controller import mdp_controller
from highway_env.road.objects import Obstacle, Landmark

Observation = np.ndarray
DEFAULT_WIDTH: float = 3.5  # width of the straight lane
...
class AbstractEnv(gym.Env):
    ...
    def __init__(self, config: dict = None) -> None:
        ...
        self.ends = [220, 100, 100, 320]  # choose, optimization, merge, after
    ...
    def reset(self, is_training=True, testing_seeds=0, num_CAV=0) -> Observation:
        ...
        # return self.seed, num1, num2, num3, num4
        ...
        if self.config["action_masking"]:
            ...
            available_actions = [[0] * self.n_a for _ in range(len(self.controlled_vehicles))]
    ...
    def _get_available_actions(self, vehicle, env_copy):
        ...
        actions = [env_copy.ACTIONS_ALL['IDLE'], env_copy.ACTIONS_ALL['BRAKE']]
    ...
    def check_safety_room(self, vehicle, action, surrounding_vehicles, env_copy, time_steps):
        ...
        para: vehicle: the ego vehicle
              surrounding_vehicles: [v_fl, v_rl, v_fe, v_re, v_fr, v_rr]
        ...
        """
        # 因为原代码只考虑了至多两根道，在这里我们需要考虑最多四根道，因此需要在surrounding_vehicles中添加自身前后的车辆
        ...
        for t in range(time_steps + 1):
            if  t == 0:
                ...
                mdp_controller(vehicle, env_copy, 1)
            safety_room = self.PERCEPTION_DISTANCE
            if vehicle.trajectories[t][0][1] > 7.1:
            ...
            if action == 0 or action == 2:
                ...
                # if vehicle.lane_index[2] in [0, 1, 2]:
                    # 这里不加绝对值，是因为surrounding_vehicles[2]为正前方车辆
                ...
                #         0]) <= safety_room:
                #         safety_room = surrounding_vehicles[2].trajectories[t][0][0] - vehicle.trajectories[t][0][0]
                #     elif surrounding_vehicles[3] and (
                #             vehicle.trajectories[t][0][0] - surrounding_vehicles[3].trajectories[t][0]
                #         [0]) <= safety_room:
                #         safety_room = vehicle.trajectories[t][0][0] - surrounding_vehicles[3].trajectories[t][0][0]

            min_time_safety_rooms.append(safety_room)
        if action == 4:
            return min(min_time_safety_rooms) + 0.5
        else:
    ...
    def safety_supervisor(self, actions):
        ...
        for vehicle, action in zip(env_copy.controlled_vehicles, actions):
            ...
            if vehicle.lane_index == ("c", "d", 3):
                ...
                priority_number -= 2*(self.ends[2] - distance_to_merging_end) / self.ends[2]
        ...
        for i, vehicle_and_action in enumerate(vehicles_and_actions):
            ...
            if vehicle.lane_index[2] in [0, 1, 2]:
                v_fe, v_re = self.road.surrounding_vehicles(vehicle, True)
                if len(self.road.network.side_lanes(vehicle.lane_index)) != 0:
                    if vehicle.lane_index[2] == 0:
                        v_fl, v_rl = None, None
                        v_fr, v_rr = self.road.surrounding_vehicles(vehicle, True,
                                                                        (vehicle.lane_index[0], vehicle.lane_index[1], 1))
                    elif vehicle.lane_index[2] == 1:
                        v_fl, v_rl = self.road.surrounding_vehicles(vehicle, True,
                                                                        (vehicle.lane_index[0], vehicle.lane_index[1], 0))
                        v_fr, v_rr = self.road.surrounding_vehicles(vehicle, True,
                                                                        (vehicle.lane_index[0], vehicle.lane_index[1], 2))
                    else:
                        if vehicle.lane_index == ("a", "b", 2) and vehicle.position[0] < self.ends[0]:
                            v_fl, v_rl = self.road.surrounding_vehicles(vehicle, True, ("a", "b", 1))
                            v_fr, v_rr = None, None 
                        elif vehicle.lane_index == ("b", "c", 2) and vehicle.position[0] < sum(self.ends[:2]):
                            v_fl, v_rl = self.road.surrounding_vehicles(vehicle, True, ("b", "c", 1))
                            v_fr, v_rr = self.road.surrounding_vehicles(vehicle, True, ("b", "c", 3))
                        elif vehicle.lane_index == ("c", "d", 2) and vehicle.position[0] < sum(self.ends[:3]):
                            v_fl, v_rl = self.road.surrounding_vehicles(vehicle, True, ("c", "d", 1))
                            v_fr, v_rr = self.road.surrounding_vehicles(vehicle, True, ("c", "d", 3)) 
                        else:
                            v_fl, v_rl = self.road.surrounding_vehicles(vehicle, True, ("d", "e", 1))
                            v_fr, v_rr = None, None
                ...
                # elif vehicle.lane_index == ("b", "c", 2) and vehicle.position[0] > self.ends[0]:
                #     v_fr, v_rr = env_copy.road.surrounding_vehicles(vehicle, ("b", "c", 3))
                else:
                    v_fr, v_rr = None, None
            ...
            # vehicle is on the ramp
                ...
                # assume we can observe the straight road on the ramp
                    ...
                    v_fl, v_rl = self.road.surrounding_vehicles(vehicle, True, ("b", "c", 2))
                else:
                    v_fl, v_rl = None, None
            ...
            for t in range(n_points):
                ...
                for v in [v_fl, v_fe, v_fr, vehicle, v_rl, v_re, v_rr]:
                ...
                if vehicle.crashed:
                    ...
                    for a in available_actions:
                        ...
                        safety_room = self.check_safety_room(vehicle_copy, a, [v_fl, v_rl, v_fe, v_re, v_fr, v_rr],
                                                             env_copy, t)
                    ...
                    for other in [v_fl, v_rl, v_fe, v_re, v_fr, v_rr]:
                        if isinstance(other, Vehicle) and other.crashed:
                            other.crashed = False
        # print("new_actions:", tuple(actions))
        return tuple(actions)
    
    def change_lane_supervisor(self, actions):
        for vehicle in self.controlled_vehicles:
            if vehicle.lane_index == ("c", "d", 3):
                if vehicle.position[0] > sum(self.ends[0:2]) + 50:
    ...
    def step(self, action: Action) -> Tuple[Observation, float, bool, dict]:
        ...
        if self.config["safety_guarantee"]:
            ...
            # print("new_action: ", self.new_action)
        if self.config["change_lane"]:
            self.new_action = self.change_lane_supervisor(action)
            if self.config["safety_guarantee"]:
                self.new_action = self.safety_supervisor(self.new_action)
        if not (self.config["safety_guarantee"] or self.config["change_lane"]):
        ...
        reward = self._reward(self.new_action)
        # reward = 0
    ...
    def distance_to_merging_end(self, vehicle):
        vehicle.lane_index = vehicle.road.network.get_closest_lane_index(vehicle.position, vehicle.heading) if vehicle.road else np.nan
    ...
    def _compute_headway_distance(self, vehicle, ):
        ...
        for v in self.road.vehicles:
            ...
            if (vehicle.lane_index != ("c", "d", 3)) and (
    ...
    def check_collision(self, vehicle, other, other_trajectories):
        ...
        if isinstance(other, Vehicle):
            if self._is_colliding(vehicle, other, other_trajectories):
                ...
                # print("Collision between vehicle", vehicle.id, "and vehicle", other.id)

        elif isinstance(other, Obstacle):
            if self._is_colliding(vehicle, other, other_trajectories):
                vehicle.speed = min([vehicle.speed, 0], key=abs)
                vehicle.crashed = other.hit = True
                # print("Collision with obstacle", vehicle.id)
    ...
    def _is_colliding(self, vehicle, other, other_trajectories):
        ...
        return utils.rotated_rectangles_intersect(
            (vehicle.position, ratio * vehicle.LENGTH, ratio * vehicle.WIDTH, vehicle.heading),
            (other_trajectories[0], ratio * other.LENGTH, ratio * other.WIDTH, other_trajectories[1]))
